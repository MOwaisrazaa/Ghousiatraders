(() => {
    ...
    return `
      <article class="card" data-product="${p.id}">
        <a class="card__media" href="product.html?id=${encodeURIComponent(p.id)}">
          <img src="${p.images?.[0] || "assets/product-qasr-al-oud.jpg"}" alt="${p.name}" width="400" height="240" loading="lazy" decoding="async" />
  ...
  const mountProductPage = () => {
    ...
    if (!root) return;
    const url = new URL(window.location.href);
    const id = url.searchParams.get("id") || "qasr-al-oud";
    const p = getProduct(id) || getProduct("qasr-al-oud");
    if (!p) return;
          ...
          ({ line, p }) => `
            <div class="cart-row" data-line="${p.id}">
              ...
              <div class="cart-row__product">
                <img class="cart-row__img" src="${p.images?.[0] || "assets/product-qasr-al-oud.jpg"}" alt="${p.name}" width="72" height="72" loading="lazy" decoding="async" />
