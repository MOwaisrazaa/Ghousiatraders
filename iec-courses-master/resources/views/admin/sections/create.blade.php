...
@section('header', 'Create New Homepage Section')
...
@section('content')
    <div style="background:rgba(10,10,10,0.82);border:1px solid rgba(212,166,88,0.16);border-radius:20px;padding:32px;max-width:760px;margin:0 auto;">
        <div style="display:flex;align-items:center;gap:12px;margin:0 0 28px;">
            <span style="font-size:0.72rem;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;color:#d4a658;">Section Details</span>
        ...
        <form action="{{ route('admin.sections.store') }}" method="POST">
            ...
            <div class="pf-row-3">
                <div class="pf-field">
                    ...
                    <input type="text" class="pf-input @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title') }}" placeholder="e.g. New Arrivals" required>
                    @error('title')
                ...
                <div class="pf-field">
                    ...
                    <input type="number" class="pf-input @error('order') is-invalid @enderror" id="order" name="order" value="{{ old('order', 0) }}" min="0" required>
                    @error('order')
                        <div class="pf-error">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="pf-field">
                    ...
                    <select class="pf-select-field @error('bg_theme') is-invalid @enderror" id="bg_theme" name="bg_theme" required>
                        <option value="dark" {{ old('bg_theme', 'dark') == 'dark' ? 'selected' : '' }}>Dark (Noir)</option>
                        <option value="ivory" {{ old('bg_theme') == 'ivory' ? 'selected' : '' }}>Ivory (Light)</option>
                    </select>
                    @error('bg_theme')
                        <div class="pf-error">{{ $message }}</div>
                    @enderror
