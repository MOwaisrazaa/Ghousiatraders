...
@section('title', 'Edit Navigation Page')
@section('header', 'Edit Navigation Page / Link')
...
@section('content')
    ...
    <div style="background:rgba(10,10,10,0.82);border:1px solid rgba(212,166,88,0.16);border-radius:20px;padding:32px;max-width:800px;">
        <div style="display:flex;align-items:center;gap:12px;margin:0 0 28px;">
            <span style="font-size:0.72rem;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;color:#d4a658;">Edit Page Link Details</span>
        ...
        <form action="{{ route('admin.pages.update', $page->id) }}" method="POST" id="page-form">
            ...
            <div class="pf-field">
                ...
                <select name="type" id="type" class="pf-input" onchange="toggleFields()">
                    <option value="system" {{ old('type', $page->type) === 'system' ? 'selected' : '' }}>System Link (Redirects to routes like /shop, /contact, or external URL)</option>
                    <option value="custom" {{ old('type', $page->type) === 'custom' ? 'selected' : '' }}>Custom Content Page (Dynamically displays custom page content)</option>
                </select>
                @error('type')
            ...
            <div class="pf-field">
                ...
                <input type="text" class="pf-input @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $page->name) }}" placeholder="e.g. ABOUT US" required>
                @error('name')
                    <div class="pf-error">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="pf-field" id="system-link-field">
                ...
                <input type="text" class="pf-input @error('link') is-invalid @enderror" id="link" name="link" value="{{ old('link', $page->link) }}" placeholder="e.g. /shop, /contact, /#signature, or https://google.com">
            ...
            <div id="custom-page-fields" style="display:none;">
                <div class="pf-field">
                    ...
                    <input type="text" class="pf-input @error('slug') is-invalid @enderror" id="slug" name="slug" value="{{ old('slug', $page->slug) }}" placeholder="e.g. brand-story">
                    <span style="font-size:0.75rem;color:rgba(248,231,208,0.45);margin-top:6px;display:block;">
                ...
                <div class="pf-field">
                    ...
                    <div id="quill-editor-wrapper" style="display: none;">
                        ...
                        <input type="hidden" id="page_content_input" name="content" value="{{ old('content', $page->content) }}">
                    </div>
                    ...
                    <div style="background: rgba(255,255,255,0.02); border: 1px solid rgba(212,166,88,0.2); border-radius: 12px; padding: 20px; max-height: 250px; overflow-y: auto;">
                        @forelse($products as $product)
                            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
                                <input type="checkbox" name="products[]" id="product_{{ $product->id }}" value="{{ $product->id }}" {{ in_array($product->id, old('products', $page->products->pluck('id')->toArray())) ? 'checked' : '' }} style="accent-color:#d4a658; width:16px; height:16px; cursor:pointer;">
            ...
            <div class="pf-field">
                <label for="order" class="pf-form-label">Sort Order</label>
                <input type="number" class="pf-input @error('order') is-invalid @enderror" id="order" name="order" value="{{ old('order', $page->order) }}" placeholder="e.g. 9">
                @error('order')
                    <div class="pf-error">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="pf-field" style="display:flex;align-items:center;gap:10px;margin-top:20px;">
                <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $page->is_active) ? 'checked' : '' }} style="accent-color:#d4a658;width:18px;height:18px;cursor:pointer;">
